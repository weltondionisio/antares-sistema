...
def carregar_dados():
    ...
    # Ajuste de escala caso necessário (mantido 1.0 por padrão; altere se precisar dividir por 1000)
    FATOR_DIVISAO = 1.0  
...
with col2:
    # Lógica Dinâmica: Se for "Ano", exibe o Total Anual. Se for um mês específico, exibe a Mediana daquele mês.
    ordem_meses = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez']
    ...
    if mes_selec == "Ano":
        ...
        st.metric("Total de Acidentes Previstos no Ano", f"{total_anual:,.2f}")
        
        # No modo "Ano", o gráfico de barras exibe a SOMA total de cada mês
        ...
        st.metric(f"Mediana de Acidentes ({mes_selec})", f"{mediana_mes:,.2f}")
        
        # No modo mês específico, o gráfico mantém a distribuição da mediana mensal
