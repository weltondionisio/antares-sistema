...
# 2. Filtro em Cascata: Estado (UF)
lista_estados = ["Todos os Estados"] + sorted(df_global['estado'].dropna().unique().tolist())
estado_selecionado = st.sidebar.selectbox("Filtrar por Estado (UF):", lista_estados)

# Filtrar municípios com base no estado escolhido
...
# 3. Filtro em Cascata: Município
lista_municipios = ["Todos os Municípios"] + sorted(df_filtrado_estado['municipio'].dropna().unique().tolist())
...
# Aplicar restrições de estado e município no mapa
if estado_selecionado != "Todos os Estados":
    df_mapa_base = df_mapa_base[df_mapa_base['estado'] == estado_selecionado]
    zoom_mapa = 5.5
    centro_lat, centro_lon = df_mapa_base['latitude'].mean(), df_mapa_base['longitude'].mean()
else:
    zoom_mapa = 3.2
...
if municipio_selecionado != "Todos os Municípios":
    ...
    if not df_mapa_base.empty:
        centro_lat, centro_lon = df_mapa_base['latitude'].values[0], df_mapa_base['longitude'].values[0]
...
with col2:
    ...
    # Filtragem correta para o gráfico de barras sem perdas de escala
    ...
    if municipio_selecionado != "Todos os Municípios":
        ...
        titulo_barras = f"Demanda - {municipio_selecionado} ({estado_selecionado})"
