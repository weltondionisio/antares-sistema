...
# Filtro de Estado Dinâmico
lista_estados = ["Todos os Estados"] + sorted(df_global['estado'].dropna().unique().astype(str).tolist())
...
# Filtro de Município Dinâmico baseado no estado
lista_municipios = ["Todos os Municípios"] + sorted(df_filtrado_estado['municipio'].dropna().unique().astype(str).tolist())
...
if not df_mapa_base.empty:
    ...
    zoom_mapa = 7 if municipio_selecionado != "Todos os Municípios" else (5.2 if estado_selecionado != "Todos os Estados" else 3.2)
else:
